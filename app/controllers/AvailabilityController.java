   ...
   /* Get user object from request */
    ...
    private List<AvailabilityModel> availableSlotsForAny(Date startDate, Date endDate, String serviceId) {
        ...
        for ( int i = 0; i < availabilities.size(); i++ ) { // For i from 0 to the size of the availabilities list size
            AvailabilityModel newAv = new AvailabilityModel(null, "any", availabilities.get(i).getStartDate(), availabilities.get(i).getEndDate(), false); // New availability model where the ID is null, the userID is any, and has the same start and end date as the original availability at i
